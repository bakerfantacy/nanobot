...
from nanobot.config.loader import DEFAULT_AGENT_NAME
...
def onboard(
    name: str = typer.Option(
        DEFAULT_AGENT_NAME, "--name", "-n",
        help="Agent name (directory under ~/.nanobot/)",
    ),
):
    ...
    config = Config()
    save_config(config, agent_name=name)
    console.print(f"[green]✓[/green] Created config at {config_path}")

    # Create workspace
    workspace = get_workspace_path(agent_name=name)
    console.print(f"[green]✓[/green] Created workspace at {workspace}")

    # Create default bootstrap files
    _create_workspace_templates(workspace)

    console.print(f"\n{__logo__} nanobot agent [cyan]{name}[/cyan] is ready!")
    console.print("\nNext steps:")
    console.print(f"  1. Add your API key to [cyan]~/.nanobot/{name}/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    if name == DEFAULT_AGENT_NAME:
        console.print("  2. Chat: [cyan]nanobot agent -m \"Hello!\"[/cyan]")
    else:
        console.print(f"  2. Chat: [cyan]nanobot agent -n {name} -m \"Hello!\"[/cyan]")
...
def _make_provider(config):
    ...
    if not (p and p.api_key) and not model.startswith("bedrock/"):
        ...
        console.print(
            f"Set one in ~/.nanobot/{config.agent_name}/config.json under providers section"
        )
        raise typer.Exit(1)
...
def gateway(
    ...
    port: int = typer.Option(None, "--port", "-p", help="Gateway port (overrides config)"),
    ...
    config = load_config(agent_name=name)

    # Port: CLI flag > config > default
    effective_port = port if port is not None else config.gateway.port

    console.print(
        f"{__logo__} Starting nanobot gateway [cyan]{name}[/cyan] on port {effective_port}..."
    )
...
def agent(
    ...
    if message:
        ...
        agent_label = f" ({name})" if name != DEFAULT_AGENT_NAME else ""
        console.print(
            f"{__logo__} Interactive mode{agent_label} "
...
def list_agents_cmd():
    ...
    from nanobot.config.loader import list_agents, load_config

    agents = list_agents()

    if not agents:
        console.print("[yellow]No agents configured.[/yellow]")
        console.print(f"Run [cyan]nanobot onboard --name <agent-name>[/cyan] to create one.")
        return

    table = Table(title="Configured Agents")
    table.add_column("Name", style="cyan")
    table.add_column("Model", style="white")
    table.add_column("Gateway Port", style="green")
    table.add_column("Workspace", style="dim")
    table.add_column("Channels", style="yellow")

    for agent_name in agents:
        try:
            config = load_config(agent_name=agent_name)
        except Exception:
            table.add_row(agent_name, "[red]error loading config[/red]", "", "", "")
            continue

        # Collect enabled channels
        enabled = []
        for ch_name in ("whatsapp", "telegram", "discord", "feishu", "dingtalk",
                        "email", "slack", "qq"):
        ...
        table.add_row(
            ...
            ", ".join(enabled) if enabled else "[dim]none[/dim]",
        )
...
def channels_status(
    ...
    table = Table(title=f"Channel Status ({name})")
...
def _get_bridge_dir() -> Path:
    ...
    # User's bridge location (shared across agents)
...
def cron_list(
    ...
    store_path = get_data_dir(name) / "cron" / "jobs.json"
    ...
    table = Table(title=f"Scheduled Jobs ({name})")
...
def status(
    ...
    console.print(f"{__logo__} nanobot Status (agent: [cyan]{name}[/cyan])\n")
